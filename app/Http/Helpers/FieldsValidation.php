...
class FieldsValidation
{
    ...
    )
    {
        ...
        //Validação NÚMERO
        if(!$this->numeroValido($numero)) throw new \Exception("Número da porta no formato inválido!");

        //Validação Telefone chamando o método de validação de telefone
        if(!$this->celularTelefoneValidos($celular)) throw new \Exception("Telefone no formato inválido!");
        ...
        if(!$this->dataValida($data_nascimento)) throw new \Exception("Data no formato inválido!");
    ...
    public function nomeValido($nome): bool
    {
    ...
    public function dataValida($data): bool
    {
    ...
    public function precoValido($preco): string
    {
